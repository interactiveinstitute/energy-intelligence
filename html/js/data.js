...
({
  ...
  getDataFromRequest: function(params, result) {
    var resample;
  ...
  setDataAndTransform: function(data, from, to, transition) {
    if (transition == null) {
      transition = true;
    }
    if (transition) {
  ...
  constructor: function(chart) {
    ...
    return this.wasteArea = d3.svg.area().x(function(d) {
      return _this.chart.x(d.resampledAt);
    }).y0(function(d) {
      return _this.chart.height - _this.chart.config.padding_bottom;
    }).y1(function(d) {
  ...
  setDataAndTransform: function(data, from, to, transition) {
    ...
    this.chart.time.select('.area').datum(data).attr('transform', from).transition().duration(transitionLength).attr('transform', to);
    this.chart.time.select('.line').datum(data).attr('transform', from).transition().duration(transitionLength).attr('transform', to);
  ...
  setDataAndTransform: function(data, from, to) {
    var bar, g,
      _this = this;
